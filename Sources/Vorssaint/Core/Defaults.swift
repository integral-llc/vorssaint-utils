...
enum UpdateHighlightsInfo {
    ...
    static func matchesRelease(_ appVersion: String) -> Bool {
        guard let version = versionParts(appVersion),
              ...
              (2...3).contains(version.prerelease.count), version.prerelease[0] == "beta",
              let number = Int(version.prerelease[1]) else { return false }
        // A hotfix of a beta, 3.4.0-beta.2.1, belongs to the same tour.
        if version.prerelease.count == 3 {
            guard let hotfix = Int(version.prerelease[2]), hotfix >= 0 else { return false }
        }
        return number >= 1
    }
