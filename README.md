...
- **Layout Switcher.** Retype a word typed on the wrong input source with one shortcut, on the selection or the word you just finished, and press it again to put the word back. Optionally correct finished words as you type, judged locally against word lists and character models that ship with the app. English and Russian are the languages it can judge; every other pair stays on the shortcut.
...
- **Transcriber.** Turn a video link or a local audio or video file into a text transcript. Decoding runs on your Mac, or on a whisper.cpp server of your own. Needs helper binaries the build stages into the app; see [contributing](CONTRIBUTING.md).
