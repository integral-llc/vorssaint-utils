...
- **Layout Switcher.** Type a word with the wrong input source active and one shortcut retypes it in the other one, on the selection, the word you are typing, or the word you just finished. The pairing is read from what each key actually produces on the layouts you have enabled, so any two of them work rather than a list somebody wrote a table for, and pressing the shortcut again puts the word back. Switch on correcting as you type to have finished words fixed without asking: each word is weighed against its spelling on the other layout, using word lists and character models that ship with the app, and rewritten only when the other spelling is clearly the better word, after which the layout follows it. A word ends at a space, at Return, or at punctuation that is punctuation on every layout you have enabled; a comma or a full stop that is a letter on another layout waits for the space, where the whole token is weighed. After Return the text is only touched when the app shows the word with a new line after it, so a message that was sent or a command that ran is never retyped, and the layout still follows. English and Russian are the languages it can judge so far; every other pair stays on the shortcut. There is a shortest-word floor, links, addresses, paths, identifiers and bracket syntax are left alone, and taking a correction back with the shortcut stops that word being corrected again. A word that is fine on both layouts is settled by the language of the sentence around it, and ordinary words you type often are learned so that names and jargon stop looking like mistakes. What it learns stays on this Mac, is left out of exported settings, never includes anything with a digit or a symbol in it, and goes with one button in settings. Nothing typed into a password field is read or remembered.
...
- **Transcriber.** Paste a video link or point it at an audio or video file you already have and it writes out a text transcript. Decoding runs on your Mac by default and nothing leaves it; if you run a whisper.cpp server of your own, you can send the audio there instead, over HTTPS unless you ask for otherwise. Transcripts land in Downloads or a folder you pick, the extracted audio is cleaned up unless you keep it, and when a site changes its player and the fetch stops working, updating that helper is one button in Settings. It needs helper binaries the build stages into the app; see [contributing](CONTRIBUTING.md) for how to build with them.
...
- Layout switcher word frequencies and character models are derived from [wordfreq](https://github.com/rspeer/wordfreq) 3.1.1 data, used under [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/)
