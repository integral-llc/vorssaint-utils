...
enum SwitcherModelFeatureTests {
    static func run(_ suite: TestSuite) {
        ...
        suite.expect(registeredDefaults[DefaultsKey.layoutSwitcherEnabled] as? Bool == false,
               "layout switcher is opt-in")
        suite.expect(registeredDefaults[DefaultsKey.layoutSwitcherAutomatic] as? Bool == false,
               "layout switcher corrects only when asked until told otherwise")
        suite.expect(registeredDefaults[DefaultsKey.layoutSwitcherMinimumWordLength] as? Int
                == Defaults.defaultLayoutSwitcherWordLength,
               "layout switcher starts at its own default word length")
        suite.expect(registeredDefaults[DefaultsKey.panelControlLayoutSwitcher] as? Bool == true,
               "layout switcher is visible in the panel when installed")
        suite.expect(registeredDefaults[DefaultsKey.transcriberLocation] as? String
                == TranscriptionLocation.local.rawValue,
               "the transcriber decodes on this Mac until told otherwise")
        suite.expect(registeredDefaults[DefaultsKey.transcriberRemoteHost] as? String == ""
        ...
        suite.expect(registeredDefaults[DefaultsKey.transcriberKeepAudio] as? Bool == false,
