...
enum FeatureCatalogTests {
    ...
    static func run(_ suite: TestSuite) {
        ...
        suite.expect(AppFeature.availabilityDefaults.count == AppFeature.allCases.count
                ...
                && AppFeature.allCases.filter {
                    $0 != .focusFollowsMouse && $0 != .fanControl && $0 != .diskImageInstaller
                        ...
                        && $0 != .youtubeTranscriber && $0 != .scrollHorizontal && $0 != .portManager
                }.allSatisfy {
        ...
        suite.expect(Set(FeaturePreset.essential.features.flatMap(\.onboardingPermissions)) == [.accessibility]
        ...
        suite.expect(activeSet(.accessibility, on: [DefaultsKey.layoutSwitcherEnabled])
        ...
        suite.expect(AppFeature.youtubeTranscriber.permissions == [.filesAndFolders]
        ...
        suite.expect(AppFeature.youtubeTranscriber.onboardingPermissions.isEmpty
        ...
        suite.expect(AppFeature.layoutSwitcher.onboardingPermissions == [.accessibility],
               "a picked layout switcher is explained under Accessibility at first launch")
        suite.expect(!AppFeature.availabilityDefaults[AppFeature.layoutSwitcher.availabilityKey].flatMap { $0 as? Bool }!,
        ...
        suite.expect(activeSet(.filesAndFolders) == [.youtubeTranscriber],
               "WhatsApp Downloads folder access stays unused until that cleaner is turned on")
        suite.expect(activeSet(.filesAndFolders, on: [DefaultsKey.whatsAppDownloadsEnabled])
        ...
        for language in AppLanguage.allCases {
            ...
            suite.expect(transcriberValues.count == 50
                    && transcriberValues.allSatisfy { !$0.isEmpty },
                   "every transcriber string is set for \(language.rawValue)")
            suite.expect(transcriberValues.allSatisfy { !$0.contains("\u{2014}") },
            ...
            suite.expect(Set(messages).count == failures.count
            ...
            let layoutSwitcherValues = Mirror(reflecting: FeatureStrings.layoutSwitcher(language))
                .children.compactMap { $0.value as? String }
            suite.expect(layoutSwitcherValues.count == 14
                    && layoutSwitcherValues.allSatisfy { !$0.isEmpty },
                   "every layout switcher string is set for \(language.rawValue)")
            suite.expect(layoutSwitcherValues.allSatisfy { !$0.contains("\u{2014}") },
        ...
        suite.expect(FeaturePreset.essential.features.isSuperset(of: [.layoutSwitcher, .youtubeTranscriber])
