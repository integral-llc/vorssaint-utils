...
enum PointerInputFeatureTests {
    static func run(_ suite: TestSuite) {
        ...
        // MARK: Transcriber
        suite.expect(YouTubeTranscriberSupport.isLocalFile("/Users/me/clip.mp4")
        ...
        suite.expectClose(YouTubeTranscriberSupport.downloadFraction(in: "[download]  42.3% of 10MiB") ?? -1,
                    0.423, "yt-dlp progress is read off its own line")
        suite.expect(YouTubeTranscriberSupport.downloadFraction(in: "some other line") == nil,
               "a line that is not progress reports none")
        suite.expectClose(YouTubeTranscriberSupport.transcribedSeconds(
            in: "[00:00:12.000 --> 00:01:15.500]  hello") ?? -1,
                    75.5, "whisper progress comes from the end of each segment")
        suite.expectClose(YouTubeTranscriberSupport.fraction(transcribed: 30, of: 120), 0.25,
                    "progress is the part of the audio already decoded")
        suite.expect(YouTubeTranscriberSupport.fraction(transcribed: 30, of: 0) == 0,
        ...
        suite.expect(YouTubeTranscriberSupport.remoteURL(from: "http://box.local", path: "v1")?
                .scheme == "http",
               "a scheme the user typed on purpose is kept")
        suite.expect(YouTubeTranscriberSupport.remoteURL(from: "  ", path: "v1") == nil,
        ...
        suite.expect(cappedLog.count == YouTubeTranscriberSupport.maximumLogLines
                && cappedLog.last == "line \(YouTubeTranscriberSupport.maximumLogLines + 49)",
               "a long run keeps the tail of its log and no more")
        suite.expect(YouTubeTranscriberSupport.diagnose(log: ["ERROR: members-only video"],
        ...
        suite.expect(YouTubeTranscriberSupport.diagnose(log: ["something nobody has a rule for"],
        ...
        suite.expect(YouTubeTranscriberSupport.diagnose(log: [], helper: .ytDlp, exitCode: 0) == .unknown,
               "a run that did not fail has no failure to name")
        suite.expect(TranscriberHelper.ytDlp.updatesItself
                && TranscriberHelper.allCases.filter(\.updatesItself).count == 1,
               "yt-dlp is the only helper that replaces itself at runtime")
        suite.expect(YouTubeTranscriberSupport.checksum(for: "yt-dlp_macos",
        ...
        suite.expect(sameGlyph.forward.isEmpty && sameGlyph.shared.contains("a"),
               "an unchanged key is shared, not a pairing")
        suite.expect(!sameGlyph.isUsable,
        ...
        let qwertz = LayoutSwitcherSupport.table(from: [0: "y", 1: "z", 2: "k", 3: "e"],
                                                 to: [0: "z", 1: "y", 2: "k", 3: "e"])
        suite.expect(qwertz.isUsable, "a pair differing on two keys is still a pair")
        expectEqual(qwertz.mapForward("keyz") ?? "", "kezy",
                    "letters both layouts agree on survive the correction")
        suite.expect(qwertz.mapForward("keyz").flatMap(qwertz.mapReverse) == "keyz",
               "a correction over shared letters still round trips")
        suite.expect(qwertz.mapForward("keke") == nil,
               "a word that would come out unchanged is not offered as a correction")
        suite.expect(LayoutSwitcherSupport.singleCharacter("\u{9}") == nil
        ...
        suite.expect(LayoutSwitcherSupport.trailingWord(in: "one two thr") == "thr"
        ...
        suite.expect(!LayoutSwitcherSupport.isCorrectable("hi", minimumLength: shortest),
        ...
        for structured in ["https://example.com", "user@example.com", "src/main.swift",
            ...
            suite.expect(LayoutSwitcherSupport.looksStructured(structured),
                   "\(structured) reads as something meant for a machine")
        }
        suite.expect(!LayoutSwitcherSupport.looksStructured("Hello")
                && !LayoutSwitcherSupport.looksStructured("word"),
               "an ordinary word is not mistaken for an identifier")
        suite.expect(Defaults.sanitizedLayoutSwitcherWordLength(0) == Defaults.defaultLayoutSwitcherWordLength
        ...
        let layoutSwitcherCode = layoutSwitcherSource.components(separatedBy: "\n")
            .filter { !$0.trimmingCharacters(in: .whitespaces).hasPrefix("//") }
            .joined(separator: "\n")
        suite.expect(layoutSwitcherCode.contains("IsSecureEventInputEnabled")
        ...
        suite.expect(!layoutSwitcherCode.contains("NSLog")
                && !layoutSwitcherCode.contains("print(")
                && !layoutSwitcherCode.contains("/tmp"),
               "layout switcher writes no typed text to a log or a file")
        suite.expect(layoutSwitcherCode.contains("TextSnippetService.postExpansion"),
               "layout switcher types back through the one shared typing routine")
        suite.expect(!layoutSwitcherCode.contains("usleep") && !layoutSwitcherCode.contains("as!"),
